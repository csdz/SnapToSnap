...
namespace VoiceChat.UI
{
    public class VoiceChatUi : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            GUILayout.Window(1, new Rect(150, 10, Screen.width-260, Screen.height - 20), Window, "", GUIStyle.none);
        }

        void Window(int id)
        {
            GUI.Box(new Rect(0, 0, Screen.width-100, Screen.height - 20), "");
            ...
            else
            {
                ...
                foreach (string device in VoiceChatRecorder.Instance.AvailableDevices)
                {
                    ...
                    if (device == null || device.Equals(""))
                        buttonText = "Default microphone (recommanded)";
